...
# @RdocFunction points
#
# @alias points,QDNAseqSignals,missing-method
#
# @title "Plot copy number profile"
#
# @synopsis
...
setMethod("points", signature(x="QDNAseqSignals"),
    function (x, y, main=NULL, includeReadCounts=TRUE,
    logTransform=TRUE, scale=TRUE, sdFUN="sdDiffTrim",
    delcol=getOption("QDNAseq::delcol", "darkred"),
    losscol=getOption("QDNAseq::losscol", "red"),
    ...
    pointcex=getOption("QDNAseq::pointcex", 0.1),
    xlab=NULL, ylab=NULL, ylim=NULL, xaxt="s", yaxp=NULL,
    showDataPoints=TRUE, showSD=FALSE, doSegments=TRUE, doCalls=TRUE, ... ) {
    ...
    for (i in seq_len(ncol(x))) {
        ...
        if (doCalls) {
            ...
            points(pos, cn, cex=pointcex, col=pointcol, main=main[i],
                xlab=NA, ylab=NA, ylim=ylim, xaxt="n", xaxs="i", yaxs="i",
                yaxp=yaxp, tck=-0.015, las=1, pch=pointpch)
        }
#        mtext(text=xlab, side=1, line=2, cex=par("cex"))
...
setMethod("noisePlot", signature=c(x="QDNAseqReadCounts", y="missing"),
    ...
    signal <- counts / fit
    signal[fit <= 0] <- 0
    noise <- apply(signal, 2, sdFUN, na.rm=TRUE,trim = 0.001)  #noise <- apply(signal, 2, sdFUN, na.rm=TRUE,trim = 0.0005)
    plot(reciprocalOfAverageUsedReadsPerBin, noise^2, main=main, cex = 2, #cex=0.9,
    #    xlim=c(0, 1.1*max(reciprocalOfAverageUsedReadsPerBin)),
    #    ylim=c(0, 1.04*max(noise^2)),
         xlab=NA, ylab=NA, xaxs="i", yaxs="i", xaxt="n", yaxt="n", xlim =c(0,0.045), ylim=c(0,0.045), ...)  # xlim =c(0,0.02), ylim=c(0,0.042), ...)
    ...
    axis(side=1, tck=-.015, at=c(300, 200, 100, 0))
    axis(side=2, tck=-.015)
    axis(side=1, tck=-.015, at=at, labels=labels)
 #    axis(side=2, lwd=0, line=-0.4)
# ORIGINAL    mtext(side=1, xlab, line=2, cex=par("cex"))
    mtext(side=1, xlab, line=3.2, cex=2)
    mtext(side=2, symbol, line=2.8, las=1, cex=2)
